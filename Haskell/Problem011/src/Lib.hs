...
neswDiagonals n =
  ...
  map (\startRow ->
         ...
         [0..(nrows values)-startRow] -- Length of diagonal is nrows-startRow+1
      )
...
nwseDiagonals n =
  map (\startCol ->
         map (\offset -> getElem (1 + offset) (startCol + offset) values)
         [0..(ncols values)-startCol] -- Length of diagonal is ncols-startCol+1
      )
  [1..(ncols values)-n+1]       -- Start at column 1 (top left corner) and march across to the right to highest column
                                -- that allows diagonals of length n to be formed.
  ++
  map (\startRow ->
         map (\offset -> getElem (startRow+offset) (1+offset) values)
         [0..(nrows values)-startRow] -- Length of diagonal is nrows-startRow+1
      )
  [2..(nrows values)-n+1]       -- Start at row 2 and march down to the bottom (well, almost)
